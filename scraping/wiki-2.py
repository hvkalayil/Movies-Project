        ...
        for line in wiki_read:
            ...
            if '<td><i>' in line:
                ...
                if len(parts) > 1:
                    ...
                    movie = parts[-1].split('</i>')[0].split('>')[-1].replace('''\n''', '')
                    url = ''
                if '/wiki/' in url:
                    ...
                    if len(result) != 0:
                        ...
                        day = oldDate[1]
                        year = oldDate[2]
                    dt = datetime.datetime(int(year)+2000,int(month),int(day))
                obj['MovieName'] = movie
            ...
            if '<td>' in line and state == 'Prodcuer':
                producers = line.split(',')
                producer = []
                for p in producers:
                    producer.append(p.split('</a>')[0].split('>')[-1].replace('<td>','').replace('''\n''', ''))
...
with open('dataStore/2007-2005.json','w') as f:
